        ...
        oneTap.render({
            ...
            try {
                if (VKID.Auth && typeof VKID.Auth.exchangeCode === 'function') {
                    ...
                    const firstName = dataBaba?.given_name || dataBaba?.first_name || '';
                    const lastName = dataBaba?.family_name || dataBaba?.last_name || '';
                    const picture = dataBaba?.picture || '';
                    const email = (dataBaba?.email || tokenRes?.email || '').toString();
                    try {
                        await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$api$2f$request$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["NetworkService"].csrf();
                    } catch  {}
...
//# sourceMappingURL=src_943397ba._.js.map