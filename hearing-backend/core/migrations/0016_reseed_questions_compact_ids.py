...
def _orig_section_for_id(orig_id: int) -> str:
    if 1 <= orig_id <= 12:
...
def forwards(apps, schema_editor):
    ...
    # Source of truth: questions from 0008_add_input_placeholder (text and options)
    QUESTIONS = [
        {
            "id": 1,
    ...
    # Подготовка: снять номера у всех вопросов, чтобы избежать конфликтов уникальности при пересоздании
    Question.objects.update(number=None)

    for q in QUESTIONS:
        qid = int(q['id'])
        section = _orig_section_for_id(qid)
    ...
    # Удалить вопросы, отсутствующие в эталонном списке
    desired_ids = [int(q['id']) for q in QUESTIONS]
    Question.objects.exclude(id__in=desired_ids).delete()

    # Migrate existing Test answers to the new compact question IDs
    for t in Test.objects.all().only('id', 'answers'):
        ...
        for a in answers:
            ...
            if mapped_qid != old_qid_int:
                ...
                changed = True
            # Обновим section в ответе исходя из оригинальной группы по старому id
            if 1 <= old_qid_int <= 12:
...
def backwards(apps, schema_editor):
    # Irreversible: compacting IDs and updating Test answers cannot be safely undone
    pass
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('core', '0015_remove_duplicates_and_renumber'),
    ]
